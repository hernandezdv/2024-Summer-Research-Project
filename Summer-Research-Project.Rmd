...
for(i in 1:length(results)) {
  mhadj_or_data <- results[[i]]$`M-H adjusted OR`
  mhadj_data <- rbind(mhadj_data, data.frame(
    Variable = names(results)[i],
    OR = mhadj_or_data[1],  #extract the OR
    CI_lower = mhadj_or_data[2],  #extract the lower bound of CI
    CI_upper = mhadj_or_data[3]   #extract the upper bound of CI
  ))
}
